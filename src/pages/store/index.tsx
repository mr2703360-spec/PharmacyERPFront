...
import { useDeleteMedicine } from "@/api";
import {
  MoreHorizontal,
  Pencil,
  Trash2,
...
export default function Index() {
  ...
  const responseData =
    medicinesResponse?.status === 200 ? medicinesResponse.data : null;
  const medicinesData = responseData?.data || [];
  ...
  // Delete mutation using Orval hook
  ...
  const lowStockCount = medicinesData.filter(
    (m) => m.storeStatus === "OutOfStock" || (m.quantity ?? 0) < 10,
  ).length;
  ...
  const columns: ColumnDef<MedicineType>[] = [
    ...
    },
    {
      ...
      header: "حالة المخزون",
      cell: ({ row }) => (
        <Badge variant="secondary">{row.original.storeStatus}</Badge>
      ),
    },
    {
      id: "actions",
      ...
      cell: ({ row }) => {
        ...
        const hasAnyAction =
          can("view_store") || can("update_store") || can("delete_store");
